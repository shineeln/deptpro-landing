...
export const FAQItem: React.FC<{ q: string; a: string; theme: string }> = ({ q, a, theme }) => {
  ...
  return (
    <div className={`mb-4 rounded-2xl border transition-all ${isDark ? 'border-slate-800 bg-slate-900/20' : 'border-slate-200 bg-white shadow-sm'}`}>
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-6 py-5 flex items-center justify-between text-left group"
      >
        <span className={`text-sm md:text-base font-bold transition-colors ${isOpen ? 'text-blue-500' : ''}`}>{q}</span>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100 pb-6' : 'max-h-0 opacity-0'}`}>
        <p className="px-6 text-sm text-slate-500 font-medium leading-relaxed pt-2">
          {a}
        </p>
      </div>
    </div>
