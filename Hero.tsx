  ...
  return (
    <header className="relative pt-32 pb-20 overflow-hidden min-h-screen flex flex-col justify-center">
      ...
      <div className="max-w-5xl mx-auto px-6 text-center relative z-10 mb-12">
        <Reveal>
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-blue-500/20 bg-blue-500/5 mb-6">
              ...
              <span className="text-[9px] font-black tracking-widest uppercase text-blue-500">{t.hero.tag}</span>
           </div>
           ...
           <p className="text-base md:text-xl text-slate-500 max-w-2xl mx-auto mb-10 font-medium">{t.hero.subtitle}</p>
      ...
      <div className="max-w-6xl mx-auto px-6 relative w-full group">
         <div className={`dashboard-card rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden min-h-[500px] flex flex-col ${theme === 'dark' ? 'bg-slate-900/40' : 'bg-white/80 backdrop-blur-xl'}`}>
            <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-10 pb-6 border-b border-slate-800/20">
               {heroSlides.map((slide, idx) => (
                  <button key={slide.id} onClick={() => setActiveSlide(idx)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeSlide === idx ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-blue-500'}`}>
            ...
            <div className="absolute top-1/2 -translate-y-1/2 left-4 md:left-8 opacity-0 group-hover:opacity-100 transition-opacity">
               <button onClick={() => setActiveSlide((activeSlide === 0 ? heroSlides.length - 1 : activeSlide - 1))} className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 hover:bg-slate-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}><ChevronLeft size={20}/></button>
            </div>
            <div className="absolute top-1/2 -translate-y-1/2 right-4 md:right-8 opacity-0 group-hover:opacity-100 transition-opacity">
               <button onClick={() => setActiveSlide((activeSlide === heroSlides.length - 1 ? 0 : activeSlide + 1))} className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 hover:bg-slate-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}><ChevronRight size={20}/></button>
            </div>
         </div>
      </div>
    </header>
